...
- **PR #19 `feat(css): PR-E — View Transitions API`** — 実装済み、レビュー/マージ待ち

## 次にやること (次セッションの出発点)

### 1. CSS 近代化ロードマップ

PR を分けて「各回で学ぶ 1 機能セット」を決める。順序は「効果 × 他 PR への影響の少なさ」。PR-A〜E が揃った。残るは scroll-driven animations (任意、効果薄ければ skip)。
