...
- **PR #16 `feat(css): PR-B — logical properties / dvh / :focus-visible / text-wrap / accent-color`** — 実装済み、レビュー/マージ待ち

## 次にやること (次セッションの出発点)

### 1. CSS 近代化ロードマップ

PR を分けて「各回で学ぶ 1 機能セット」を決める。順序は「効果 × 他 PR への影響の少なさ」。PR-A〜B が揃ったので、以降のトークン参照先は `:root` の `--color-*` / `--space-*` / `--radius-*` / `--font-*` に統一済み。ブロック軸指定も `margin-block` / `padding-block` / `border-block-end` の logical property で書く。
