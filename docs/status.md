...
これまで playwright MCP で手元の動作確認を都度行ってきたが、main への push と PR check 時に同じフローを自動で回したい。

- Playwright + CDP Virtual Authenticator で「登録 → ログイン → 猫作成 → トイレ記録 → ログアウト」を 1 本
- `DEV_BYPASS_USER_ID` を使う dev モードで最短のシナリオ先行、WebAuthn は後追いでも可
- CI (`.github/workflows/check.yml` 相当) に Playwright ステップを追加。別 workflow 化するかは要検討
- `packages/web` 配下に `e2e/` ディレクトリを切る案 (unit テストはまだないので共存設計を同時に考える)
- staging Worker は当面立てず、ローカル `vp dev` 相手で先に形にしてから本番直接 smoke の是非を判断
...
### 3. PR #11 phase 2: `created_by` backfill + NOT NULL 化 (運用 TODO)
