...
- **PR #15 `feat(css): PR-A 基盤 — @layer / OKLCH デザイントークン / ダークモード`** — ロードマップ (status.md) のみコミット済み、**実装 (CSS 書き換え) は未着手**。別セッションで続きを書く想定なので、このブランチ (`docs/status-css-roadmap`) は PR-A 実装ブランチに転用する。次セッションは `git switch docs/status-css-roadmap` → `git pull` で入って `packages/web/src/index.css` の書き換えから再開する

## 次にやること (次セッションの出発点)

### 1. CSS 近代化ロードマップ

PR を分けて「各回で学ぶ 1 機能セット」を決める。順序は「効果 × 他 PR への影響の少なさ」。

#### PR-A: `@layer` + デザイントークン + ダークモード (基盤) — **PR #15 作成済み / 実装待ち**

現状: ロードマップ docs のみコミット済み。ブランチ `docs/status-css-roadmap` に実装を足していく。コミット後は PR タイトルとブランチ名の乖離が気になるなら、別途 `feat/css-pr-a-tokens` などに切り直して PR #15 を close → 再 open でもよい。

実践する機能:

- `@layer reset, tokens, base, components, utilities` でカスケード階層を明示
- `:root` に CSS custom properties でトークン化 (`--color-*` / `--space-*` / `--radius-*` / `--font-*`)
- `oklch()` で色を表現、`color-mix(in oklch, ...)` で hover / disabled を派生
- `@property --foo` で型付きカスタムプロパティ (transition 可能な色・数値)
- `@media (prefers-color-scheme: dark)` で自動ダークモード (トークン差し替えだけで効く構造)
- 既存のハードコード色 (`#2563eb` / `#1a1a1a` / `#fafafa` 等) を全てトークンに置換

成功基準: `index.css` 内の色リテラルは OKLCH を `--color-*` に束ねた 1 箇所のみ。ダーク切替で全画面が破綻しない。

次セッションの入口:

```bash
git switch docs/status-css-roadmap
git pull
# packages/web/src/index.css を @layer + :root トークン化から書き始める
pnpm dev   # http://localhost:5173 で実機確認しながら進める
```
